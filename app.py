...
app.config['MAIL_USERNAME'] = 'keshavmittal10767@gmail.com'
app.config['MAIL_PASSWORD'] = 'nvey usbv ykfg ahcy'
...
# epoches: Number of times model trined over a dataset 
@app.route('/')
def index():
...
def sendmessage_async(recipients, subject, body):
    def send_message():
        with app.app_context():
            msg = Message(
                subject,
                sender='keshavmittal10767@gmail.com',
...
def gen_new(video):
    ...
    while True:
        ...
        if red_count==5:
            ...
            #if not messagesent:
                #messagesent=True
            sendmessage_async(['keshavmittal10768@gmail.com'],'High Crowd Detected in Station',str(no_faces)+' persons detected in station. Please take necessary action')
...
def gen_new1(video):
    ...
    while True:
        ...
        if red_count==5:
            ...
            #if not messagesent:
                #messagesent=True
            sendmessage_async(['keshavmittal10768@gmail.com'],'Uncleanliness Detected in Station',str(no_waste)+' trash detected in station. Please clean the area.')
            cv2.putText(frame, "High Alert Message Sent!!", (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 3)
        elif green_count==5:
            status = "Green"
            clor = (0, 255, 0)
...
def gen_new3(photo):
    ...
    if red_count==5:
        ...
        #if not messagesent:
            #messagesent=True
        sendmessage_async(['keshavmittal10768@gmail.com'],'High Crowd Detected in Station',str(no_waste)+' persons detected in station. Please take necessary action.')
        cv2.putText(frame, "High Alert Message Sent!!", (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 3)
    elif green_count==5:
        status = "Green"
        clor = (0, 255, 0)
